...
def compute_stealth_address(stealth_pub):
    """Compute Starknet address from stealth public key (simplified)"""
    # Use x-coordinate as the address (truncated to 251 bits)
    address = stealth_pub[0] % (2**251)
    return hex(address)

async def main():
    parser = argparse.ArgumentParser(description='Claim stealth funds')
    parser.add_argument('--view-priv', required=True, help='View private key (hex)')
    parser.add_argument('--spend-priv', required=True, help='Spend private key (hex)')
    parser.add_argument('--ephemeral-x', required=True, help='Ephemeral pubkey X (hex)')
    parser.add_argument('--ephemeral-y', required=True, help='Ephemeral pubkey Y (hex)')
...
""")
    ...
    print(f"  Salt: 0x{stealth_priv % (2**251):064x}")
